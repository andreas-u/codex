...
class CampaignRepository @Inject constructor(private val jdbi: Jdbi) {
    ...
    fun listByGm(gmId: UUID): List<Campaign> =
        jdbi.withHandle<List<Campaign>, Exception> { handle ->
            handle.createQuery("SELECT id, title, status, started_on, gm_id, setting_id, calendar_id FROM campaign WHERE gm_id = :gmId")
                .bind("gmId", gmId)
                .map(CampaignMapper())
                .list()
        }
    ...
    fun findById(id: UUID): Campaign? =
        jdbi.withHandle<Campaign?, Exception> { handle ->
            handle.createQuery("SELECT id, title, status, started_on, gm_id, setting_id, calendar_id FROM campaign WHERE id = :id")
                .bind("id", id)
                .map(CampaignMapper())
                .findOne()
                .orElse(null)
    ...
    fun findByIdForGm(id: UUID, gmId: UUID): Campaign? =
        jdbi.withHandle<Campaign?, Exception> { handle ->
            handle.createQuery("SELECT id, title, status, started_on, gm_id, setting_id, calendar_id FROM campaign WHERE id = :id AND gm_id = :gmId")
                .bind("id", id)
                .bind("gmId", gmId)
                .map(CampaignMapper())
                .findOne()
    ...
    fun persist(campaign: Campaign) {
        jdbi.useHandle<Exception> { handle ->
            handle.createUpdate("INSERT INTO campaign (id, title, status, started_on, gm_id, setting_id, calendar_id) VALUES (:id, :title, :status, :startedOn, :gmId, :settingId , :calendarId)")
                .bind("id", campaign.id)
                ...
                .bind("calendarId", campaign.calendar?.id)
                .execute()
        }
    }
